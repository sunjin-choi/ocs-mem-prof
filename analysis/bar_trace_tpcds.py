...
def parse_and_format_filename(filename):
    ...
    # if formatted is "Single Column Aggregation Group By", then return "Group By"
    if formatted_segment == "Single Column Aggregation Group By":
        return "Group By"
...
for trace_file in unique_trace_files:
    for cache_name in unique_cache_names:
        ...
        subset = all_data[
            ...
            & (all_data[" Trace File"].str.contains("10000"))
        ]
...
colors = ["#1f77b4", "#ff7f0e", "#8c564b", "#d62728", "#9467bd"]  # Modified hex codes
metric_columns = [
    "DRAMAccesses",
    "NFMHits",
    # "NFMMisses",
...
group_width = n_caches * bar_width + gap_width
...
for i, trace_file in enumerate(trace_files):
    for idx, cache_name in enumerate(cache_names):
        ...
        bottom_pos = 0
        bar_position = bar_l[i] + idx * bar_width
...
# CacheName labels (directly under the bars)
# ax.set_xticks([bar + bar_width/2 for bar in np.concatenate([np.arange(start, start + n_caches * bar_width, bar_width) for start in bar_l])])
ax.set_xticks(
    [
        bar
        for bar in np.concatenate(
            [
                np.arange(start, start + n_caches * bar_width, bar_width)
                for start in bar_l
            ]
        )
    ]
)
# ax.set_xticklabels(['Cache' + str(i+1) for i in range(n_caches)] * n_trace_files, rotation=0, horizontalalignment='center')
ax.set_xticklabels(
    [f"{i+1}" for i in range(n_caches)] * n_trace_files,
    rotation=0,
...
# ax2.set_xticks([group_width * i + group_width / 2 - bar_width / 2 for i in range(n_trace_files)])
# ax2.set_xticks([group_width * i for i in range(n_trace_files)])
ax2.set_xticks(
    [
        group_width * i + n_caches * bar_width / 2.0 - bar_width / 2
        for i in range(n_trace_files)
    ]
)
# ax2.set_xticks([group_width * i - bar_width / 2 for i in range(n_trace_files)])
...
plt.title("Single Op Micro-Benchmark", fontsize=20, fontname="Times New Roman")
